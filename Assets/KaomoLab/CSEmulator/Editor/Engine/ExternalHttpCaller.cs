...
namespace Assets.KaomoLab.CSEmulator.Editor.Engine
{
        ...
        : EmulateClasses.IExternalCaller
    {
        ...
        IJintCallback<string, string, string> Callback = new JintCallback<string, string, string>();
        ...
        public async void CallExternal(ExternalEndpointId endpointId, string request, string meta)
        {
            var url = options.GetUrl(endpointId.id);
            if(url == null || url == "")
            {
                logger.Error($"[{endpointId.id}]に対応したURLがありません。");
                return;
            }
            using (var client = new HttpClient())
            {
                ...
                if (result.StatusCode != System.Net.HttpStatusCode.OK)
                {
                    ...
                    var reason = string.Format("{0}:{1}:{2}", result.StatusCode, result.ReasonPhrase, url);
                    Callback.Execute(null, meta, reason);
                    logger.Error(reason);
                    return;
                }
                ...
                Callback.Execute(response.response, meta, null);
        ...
        public void SetCallEndCallback(IJintCallback<string, string, string> Callback)
        {
