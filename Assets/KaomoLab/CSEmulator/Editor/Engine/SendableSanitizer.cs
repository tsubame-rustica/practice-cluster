...
namespace Assets.KaomoLab.CSEmulator.Editor.Engine
{
        ...
        : ISendableSanitizer
    {
        ...
        {
            //★PlayerSendableSanitizerの方も忘れずに
            ...
            else if (value.GetType().IsArray)
            {
                ...
                {
                    var sanitized = Sanitize(o, SanitizeItemHandle, SanitizeItemPlayerHandle);
                    //arrayはnullにして保持する。
            ...
            else if (value is ItemHandle itemHandle)
            {
                if (SanitizeItemHandle == null)
                    return itemHandle;
                return SanitizeItemHandle(itemHandle.csItemHandler);
            }
            ...
            else if (value is ItemId itemId)
            {
                if (SanitizeItemHandle == null)
                    return itemId;
                return SanitizeItemHandle(itemId.csItemHandler);
            }
            else if (value is PlayerId playerId)
            {
                //PlayerIdはおそらくPlayerHandleになる
                if (SanitizeItemPlayerHandle == null)
                    return playerId;
                return SanitizeItemPlayerHandle(playerId.playerHandle);
            }
            else if (value is Jint.Runtime.Interop.ObjectWrapper wrapped)
            {
                //JsValue.FromObjectでwrapされたら剥がして再送
                var v = Sanitize(wrapped.Target, SanitizeItemHandle, SanitizeItemPlayerHandle);
                return v;
            }
            else if (value is Jint.Native.Object.ObjectInstance oi)
            {
                ...
                {
                    var sanitized = Sanitize(oi[key], SanitizeItemHandle, SanitizeItemPlayerHandle);
            ...
            else if (value is System.Dynamic.ExpandoObject eo)
            {
                ...
                {
                    var sanitized = Sanitize(kv.Value, SanitizeItemHandle, SanitizeItemPlayerHandle);
                    //objectのundefinedは無視する。
                    if (sanitized is Jint.Native.JsUndefined) continue;
                    var jv = Jint.Native.JsValue.FromObject(engine, sanitized);
                    ret.Set(kv.Key, jv);
